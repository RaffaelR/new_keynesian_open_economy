...
phi_pi_f_star=3;          % Taylor rule inflation
...
Tau_star= rho_tau*Tau_star(-1)+(1-rho_tau)*steady_state(Tau_star)+(1-rho_tau)*gamma_tau*(D_star/Y_star-steady_state(D_star)/steady_state(Y_star));
...
%Eqn 51: Welfare
WF2= C_star^(1-sigma_c)*Epsi_c_star^(sigma_c)/(1-sigma_c)+G_star^(1-sigma_c)/(1-sigma_c)-N_star^(1+sigma_n)/(1+sigma_n)+betta*WF2(+1);
...
X1= (C^(-sigma_c))*N*W/(1-kappa*betta) ;         % price variable 1 
X2=  (C^(-sigma_c))*N/(1-kappa*betta);         % price variable 2
X1_star= (C_star^(-sigma_c))*N_star*W_star/(1-kappa*betta) ;         % price variable 1 
...
WF2= (1/(1-betta))*(C_star^(1-sigma_c)*Epsi_c_star^(sigma_c)/(1-sigma_c)+G_star^(1-sigma_c)/(1-sigma_c)-N_star^(1+sigma_n)/(1+sigma_n));
end;
