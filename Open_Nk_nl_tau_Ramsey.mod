...
D_norm            % home Government Debt
D_norm_star            % foreign Government Debt
S            % Terms of Trade
...
D_norm=R*(S^(nu/2-1))*((D_norm(-1)*(S(-1)^(1-nu/2))/Pi_H)+G-Tau*W*N);
D_norm_star=R_star*(S^(1-nu/2))*((D_norm_star(-1)*(S(-1)^(nu/2-1))/Pi_F_star)+G_star-Tau_star*W_star*N_star);
...
%Tau= rho_tau*Tau(-1)+(1-rho_tau)*steady_state(Tau)+(1-rho_tau)*gamma_tau*(D_norm/Y-steady_state(D_norm)/steady_state(Y));
%Tau_star= rho_tau_star*Tau_star(-1)+(1-rho_tau_star)*steady_state(Tau_star)+(1-rho_tau_star)*gamma_tau_star*(D_norm_star/Y_star-steady_state(D_norm_star)/steady_state(Y_star));
...
N=find_n1(V,V_star,Tau,Tau_star,nu,sigma_n,sigma_c);            % hours
N_star=find_n2(V,V_star,Tau,Tau_star,nu,sigma_n,sigma_c);            % hours
Y=A*N/V;
Y_star=A_star*N_star/V_star;
...
D_norm=(G-Tau*W*N)*Pi_H/R;            % Government D_normebt
D_norm_star=(G_star-Tau_star*W_star*N_star)*Pi_F_star/R_star;            % Government Debt
...
%steady;
ramsey_policy(planner_discount=betta,order=1,instruments=(R,R_star,Tau,Tau_star),irf=50) C C_star Y Y_star N N_star R R_star Pi_H Pi_F_star S Tau Tau_star D_norm D_norm_star V V_star;